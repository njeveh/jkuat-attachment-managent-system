...
class AdvertView extends Component
{
    ...
    public function deleteAdvert()
    {
        try{
            $advert = Advert::find($this->advert_id);
            $attachees = $advert->attachees()->get();
            $attachees = $attachees->where('status', '!=', 'completed');
            if ($attachees->count() > 0) {
                $this->feedback_header = 'Error Deleting!!';
                $this->feedback = 'This advert can not be deleted. There are still active attachees related to it.';
                $this->alert_class = 'alert-danger';
                $this->alert_class = 'alert-danger';
                $this->dispatchBrowserEvent('advert_action_feedback');
